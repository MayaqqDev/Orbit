...
class GlobalConfigScreen(val previousScreen: Screen? = null) : Screen(Text.trans("config.orbit.global")) {
    private var layout: HeaderAndFooterLayout = HeaderAndFooterLayout(this)
    ...
    override fun init() {
        ...
        buttonCountWidget.withSize(40, 20)
        buttonCountWidget.withFilter { it.isEmpty() || (it.toIntOrNull()?.let { value -> value >= 0 } == true) }
    ...
    override fun onClose() {
        OrbitConfig.CONFIG.buttonCount = buttonCountWidget.value.toIntOrNull()?.coerceAtLeast(0) ?: buttonCount.coerceAtLeast(0)
