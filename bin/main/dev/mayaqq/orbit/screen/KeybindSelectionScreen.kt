...
class KeybindSelectionScreen(orbitButton: OrbitButton) : OrbitBaseScreen(orbitButton, Text.trans("orbit.screen.keybindselect")) {
    ...
    private var list = ListWidget(0, 0)

    override fun populate() {
        categoryMap = mutableMapOf()
        list = ListWidget(min(layout.width, 200), layout.height - layout.headerHeight - layout.footerHeight)
        ...
        categoryMap.forEach { (category, buttons) ->
            val separatorWidget = Widgets.text(Text.EMPTY.append(category.label()).withColor(Color.decode("#D0D1D4").rgb))
            separatorWidget.setSize(min(layout.width, 200), 15)
            separatorWidget.withCenterAlignment()
            list.add(separatorWidget)
